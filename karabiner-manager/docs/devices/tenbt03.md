...
### Simple Modifications（3件）

`profiles[].devices[]` 内の TENBT03 デバイスエントリに登録されています。

| From | To | メモ |
|------|------|------|
| `f4` | `fn` | 物理 **F4** キー。単独では OS で特別な動作をしないことが多いため、用途は要確認 |
| `left_option`（物理 Alt / Opt） | `consumer_key_code: mute` | Alt キーをミュートに |
| `left_command`（物理 Ctrl / Cmd） | `mission_control` | TENBT03 は mac モードで物理 Ctrl が `left_command` を送出するため、**Ctrl(Cmd) キーで Mission Control が開く**。これにより複合ルール「⌘短押しで IME」には到達しない（二重保険として `device_unless` にも登録済み） |
...
| **F4** / 電卓（Windows 時に電卓起動）     | `f4`（Simple Mod で `fn` に変換） |
| **半/全** / 英かな                         | `grave_accent_and_tilde` / `japanese_eisuu` 等（モード差あり） |
| **Alt** / Option                           | `left_option`（Simple Mod で `mute`(consumer) に変換） |
| **Ctrl** / Cmd                             | ハード仕様で `left_command` を送出（Simple Mod で `mission_control` に変換） |
| **FN**                                     | **イベント非発行（ハードウェア処理）** |
| **Shift**                                  | `left_shift` |
| **Tab**                                    | `tab` |
...
- **Ctrl(Cmd) キーのハード仕様**: TENBT03 は mac モード時に物理 Ctrl キーの HID 送出を `left_command` に入れ替える。そのためグローバルルール「⌘ 短押しで IME」に巻き込まれて英数が出る副作用があった。2026-04-21 時点で **TENBT03 をそのルールの `device_unless` に追加して除外済み**、さらに Simple Mod で `left_command`→`mission_control` を設定したため、TENBT03 の物理 Ctrl(Cmd) キーは **Mission Control を開く**動作に変わっている。
